...
# ANALYSIS_MODEL: TRL 추론 정확도가 중요 → 고품질 모델 권장
# 우선순위: ANALYSIS_MODEL > LLM_MODEL > 기본값(gpt-4o)
LLM_MODEL = os.environ.get("ANALYSIS_MODEL") or os.environ.get("LLM_MODEL", "gpt-4o")
...
- 기술: {technologies}
- 자사: {self_company} ← 이 회사가 보고서를 작성하는 주체입니다. company 필드에 "{self_company}"로 표기
- 경쟁사: {competitors} ← 자사와 구별하여 분석할 것

**[주의] 자사({self_company})와 경쟁사 구분**
- 자사 증거는 "▶ 자사 증거" 섹션에, 경쟁사 증거는 "▶ 경쟁사 증거" 섹션에 있습니다
- 자사 TRL 판정 시 자사 증거를 우선 참조하고, 공개된 보도·발표를 근거로 사용할 것
- 경쟁사 TRL 판정은 각사 해당 증거만 참조하고 자사 증거와 혼용하지 말 것

**회사별·기술별 증거 건수 (시스템 자동 집계 — evidence_count에 이 수치를 반영할 것)**
{evidence_counts}
...
**메모리 제조사 여부 판단 규칙 (엄수)**
- DRAM/HBM을 직접 제조·양산하는 기업(SK Hynix, Samsung, Micron)만 HBM/DRAM 관련 기술의 TRL 7-9 가능
- Intel, AMD, NVIDIA 등 팹리스 또는 비메모리 제조사는 해당 메모리 기술을 직접 양산하지 않으므로:
  - 표준화 단체 참여, 스펙 발표, 규격 지원 선언은 TRL 3-5 수준으로 판정
  - 실제 메모리 모듈·칩 양산 증거가 있을 때만 TRL 6 이상 인정
...
**언어 규칙**: rationale 필드는 반드시 한국어로 작성할 것. 영어 사용 금지.
...
**investment_signal 작성 규칙**:
- TRL 추정표의 rationale 또는 snippet에서 채용 공고, 투자 발표, 파트너십, M&A, 논문·특허 급증 등 구체적 신호를 찾아 1줄로 요약
- 신호가 없으면 "-" 로 기재
- 한국어로 작성
...
[
  {{
    ...
    "rationale": "위협 판정 근거 1-2줄 (TRL 수치 언급 포함, 한국어로 작성)",
    "investment_signal": "채용·투자·파트너십 등 상향 신호 1줄 또는 '-'"
  }}
]
"""
...
def _format_company_evidence_block(
    company: str,
    items: list[tuple[int, dict]],
    is_self: bool = False,
) -> list[str]:
    """회사별 증거 블록을 포맷. 자사/경쟁사 구분 레이블 포함."""
    label = f"★ 자사 — {company}" if is_self else company
    lines = [f"\n### {label} 관련 증거 ({len(items)}건)"]
    for idx, item in items:
        source_tag = ""
        if item.get("source_type") == "paper":
            source_tag = " [논문]"
        elif item.get("source_type") == "patent":
            source_tag = " [특허]"
        lines.append(
            f"  [{idx + 1}]{source_tag} {item.get('title', '')} ({item.get('date', 'N/A')})\n"
...
def _group_evidence_by_company_indexed(
    ...
    자사(self_company)와 경쟁사를 구분하여 섹션 분리.
    인덱스 미구축 시 기존 전체 전달 방식으로 폴백.
    """
    competitors = [c for c in all_companies if c != self_company]

    if not ev_idx.is_index_ready():
        print("[Analysis] evidence_index 미구축 — 기존 전체 전달 방식 사용")
        return _group_evidence_by_company(evidence_store, all_companies, technologies)

    # 전체 회사×기술 조합별 쿼리
    grouped: dict[str, list[tuple[int, dict]]] = {c: [] for c in all_companies}
    seen_urls: dict[str, set] = {c: set() for c in all_companies}

    for company in all_companies:
        for tech in technologies:
            query = f"{company} {tech} semiconductor technology TRL development production"
    ...
    # ── 자사 섹션 (별도 구분) ─────────────────────────────────────
    if self_company and self_company in grouped:
        lines.append("\n## ▶ 자사 증거")
        self_items = grouped[self_company]
        tech_counts = {
            t: sum(
                1 for _, it in self_items
                if t.lower() in (it.get("title", "") + " " + it.get("snippet", "")).lower()
            )
            for t in technologies
        }
        tech_str = ", ".join(f"{t}: {tech_counts[t]}건" for t in technologies)
        count_parts.append(f"- {self_company} (자사): 총 {len(self_items)}건 ({tech_str})")
        lines.extend(_format_company_evidence_block(self_company, self_items, is_self=True))
    ...
    for company in competitors:
        ...
        tech_str = ", ".join(f"{t}: {tech_counts[t]}건" for t in technologies)
        count_parts.append(f"- {company}: 총 {len(items)}건 ({tech_str})")
        lines.extend(_format_company_evidence_block(company, items, is_self=False))
...
def analysis_agent(state: AgentState) -> dict:
    ...
    evidence_by_company, grouped, evidence_counts = _group_evidence_by_company_indexed(
        analysis_companies, technologies, evidence_store, self_company=self_company,
    )
