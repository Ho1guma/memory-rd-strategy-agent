...
**[데이터 A] 경쟁사 비교표 (시스템이 TRL 추정표에서 자동 생성 — 그대로 삽입)**
{prebuilt_tables}
...
**# 1. 분석 배경**
- 왜 지금 HBM4, PIM, CXL을 분석해야 하는지 구체적으로 서술
- 시장 동향 데이터는 [데이터 C]의 증거를 인용하여 뒷받침 (수치 임의 생성 금지)
- 최소 3개 이상의 [N] 인용 포함
- 분량: 최소 10줄 이상

**# 2. 분석 대상 기술 현황**
SK Hynix 중심으로 각 기술별 현황을 서술합니다.
- **1) HBM**: SK Hynix의 현재 기술 수준, 개발 방향, 시장 내 위치. [N] 인용 필수.
- **2) PIM**: SK Hynix의 현재 기술 수준, 개발 방향. [N] 인용 필수.
- **3) CXL**: SK Hynix의 현재 기술 수준, 개발 방향. [N] 인용 필수.
- 각 기술 섹션은 최소 5줄 이상
...
- **1) HBM**
  - 1-1) [데이터 A]의 HBM 비교표 삽입
  - 1-2) 주요 경쟁사별 세부 분석 (증거가 있는 경우만, 최소 3줄씩, [N] 인용 필수)
- **2) PIM** (동일 구조)
- **3) CXL** (동일 구조)
- 모든 경쟁사({competitors})가 본문에 최소 1회 언급되어야 함
- SK Hynix는 2절(기술 현황)에서 자사 관점으로 다루므로 비교표에는 경쟁사만 포함

**# 5. 전략적 시사점**
- 단기 (1-2년): 즉시 실행 가능한 R&D 과제 3개 이상
- 중기 (3-5년): 기술 포지셔닝 및 투자 방향 3개 이상
- 각 제언은 앞 섹션의 분석 결과와 연결하여 논거 제시
...
def _build_prebuilt_tables(trl_table: list, threat_matrix: list, technologies: list) -> str:
    ...
    for tech in technologies:
        rows = [r for r in trl_table if r.get("technology", "").upper() == tech.upper()
        ...
        for row in rows:
            ...
            # 기술별 위협 수준 조회 (company+technology)
            threat = (
                threat_by_combo.get((company, tech.upper()))
                or threat_by_combo.get((company, tech.lower()))
                or "-"
            )
            rationale = row.get("rationale", "")[:80]
            lines.append(f"| {company} | {trl_range} | {label} | {threat} | {rationale} |")
...
def report_agent(state: AgentState) -> dict:
    ...
    prebuilt_tables = _build_prebuilt_tables(trl_table, threat_matrix, technologies)
