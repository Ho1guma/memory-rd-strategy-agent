...
### 2.3 Success Criteria (판정 가능한 기준)
...
|---------|------|------|-----------|-----------------|
| SC1-1 | HBM4, PIM, CXL 각각 | 출처·날짜가 붙은 요약 카드 ≥ N건(예: 기술당 공신력 출처 5건 이상) | 기술 현황 섹션 작성 시 근거로 인용 가능 | 포함: 논문·학회·특허·공식 보도·IR; 제외: 익명 커뮤니티 단독 주장 |
| SC1-2 | 수집 문서 메타데이터 | 키워드·엔티티(기업·제품) 태깅 완료 | 검색·재랭킹·중복 제거에 사용 | 포함: 모든 수집 스니펫; 제외: 텍스트 추출 불가 문서(이미지 전용 PDF 등) |

#### G2 관련

| 기준 ID | 대상 | 변화 | 결정·행동 | 범위 |
| --- | --- | --- | --- | --- |
| SC2-1 | 사전 정의 경쟁사 목록(구성 시 확정) | 각사별 TRL 1–9 중 **하나의 구간** + 근거 링크/인용 ≥ 2개 | 경쟁사 비교 표·서술에 사용 | TRL 4–6은 “추정” 라벨 및 간접 지표 명시(아래 TRL 절) |
| SC2-2 | 동일 목록 | 위협 수준(낮음/중간/높음) + 정의된 판정 규칙 1줄 이상 [※1] | 전략 시사점 우선순위 논의 | 규칙은 README 또는 보고서 부속에 고정 |

> **[※1] 위협 등급 판정 기준(초안)**: TRL 구간(≥7 → 높음, 5–6 → 중간, ≤4 → 낮음)을 기본으로, 투자·채용 증가 추세가 확인되면 한 단계 상향. 최종 규칙 수치는 README에 고정.

#### G3 관련

| 기준 ID | 대상 | 변화 | 결정·행동 | 범위 |
|---------|------|------|-----------|------|
| SC3-1 | 최종 보고서 | SUMMARY 1/2페이지 이내 + 필수 목차 4절 + REFERENCE | R&D 리뷰 미팅 자료로 제출 | 언어·단위·회사명 표기 규칙 통일 |
| SC3-2 | REFERENCE | 본문 인용과 1:1 대응하는 URL/DOI/제목 | 출처 검증 가능 | 포함: 본문에서 인용된 모든 자료; 제외: 인용 없이 단순 참고한 자료 |

*(N, 경쟁사 목록, 위협 등급 규칙은 구현 착수 시 README에 수치·정의로 고정)*

### 2.4 Plan — Task 분해 (Outcome 중심 실행 단위)

| Task ID | Outcome (완료 시 상태) | Success Criteria 연결 | 의존성 |
|---------|------------------------|------------------------|--------|
| T1 | 분석 범위·경쟁사 후보·키워드 세트가 확정된 상태 | `scope.yaml` 내 technologies[], competitors[], keywords[], n_evidence_min 모두 채워진 상태로 자동 판정 | — |
| T2 | 웹/문서에서 근거가 있는 스니펫이 저장·메타데이터화된 상태 | SC1-1, SC1-2 | T1 |
| T3 | 로컬/인덱스 기반 검색으로 질의별 관련 청크가 반환되는 상태; **임베딩·Retrieval 선정 평가(Hit Rate@K/MRR)는 T3 없이는 수행 불가하므로 사실상 필수** | SC1-2, Hit Rate@K/MRR | T2 |
| T4 | 기술별·회사별 TRL 추정표(추정 구간 명시) 초안 | SC2-1 | T2 |
| T5 | 위협 수준 매트릭스 초안 | SC2-2 | T4 |
| T6 | 보고서 목차에 맞춘 통합 초안 | SC3-1 | T4, T5 |
| T7 | REFERENCE 정합성·SUMMARY 압축 | SC3-2, SC3-1 | T6 |

### 2.5 Control Strategy

| 구간 | 전략 | 설명 |
|------|------|------|
| T1 완료 후 | **Conditional branch** | 범위 과대(경쟁사 > 5개 또는 기술 > 3개) 시 축소 분기; 범위 적정 시 T2 진입 |
| T2–T3 | **Loop + Retry (최대 3회)** | SC1-1 미달(출처 N건 미충족) 또는 중복률 > 50% 시 쿼리 재작성 후 재검색; 3회 초과 시 **Escalation** |
...
```
[User Query] → [Supervisor] ← 모든 에이전트가 결과를 반환하는 허브
                    │
                    ├─► Scope Agent    → scope.yaml 반환 → Supervisor
                    │
...
| **Scope** | T1 | 프로젝트 목적·분석 요건 | `scope.yaml` (technologies[], competitors[], keywords[], n_evidence_min, max_competitors) |
| **Retrieve** | T2, T3 | 쿼리, 인덱스 | `chunks[]` + 메타데이터 |
| **WebSearch** | T2 (병렬) | 쿼리 세트 | Tavily 결과 정규화 |
| **Analysis** | T4, T5 | 근거 풀 | TRL 표, 위협 매트릭스 |
| **Report** | T6, T7 | 분석 산출 | 최종 Markdown/PDF 등 |
...
- **State 필드 정의**
  ...
  | 필드 | 타입 | 설명 |
  | --- | --- | --- |
  | `scope` | object | Scope Agent 출력 (technologies, competitors, keywords, n_evidence_min) |
  | `evidence_store` | list[dict] | 수집 스니펫 (url, title, date, snippet, domain, keywords, entities) |
  | `iteration_count` | int | 현재 T2 재시도 횟수 (최대 3) |
  ...
  | `last_error` | str \| null | 직전 실패 사유 (에스컬레이션 시 출력) |

- **Graph (LangGraph 등 가정 시)**

  ```text
  User Query → Supervisor

  Supervisor → Scope Agent         : T1 범위 확정 요청
  Scope Agent → Supervisor         : scope.yaml 반환

  Supervisor → Retrieve Agent      : T2 문서 검색 지시
  Supervisor → WebSearch Agent     : T2 웹 검색 지시
...
| 리스크 | 완화 |
|--------|------|
| TRL 4–6 과신 서술 | 추정 라벨·간접 지표·한계 절 필수 |
| 확증 편향 | Tavily 다각도·반증 쿼리·출처 다양성 규칙 |
| 환각 인용 | REFERENCE와 본문 교차 검증, 가능한 원문 스니펫 보관 |
