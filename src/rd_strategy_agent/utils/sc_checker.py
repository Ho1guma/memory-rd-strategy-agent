...
def check_sc1_2(state: AgentState) -> str:
    """SC1-2: evidence items must include metadata fields; empty tags are allowed if explicitly marked."""
    for ev in state["evidence_store"]:
        if "keywords" not in ev or "entities" not in ev:
            return "fail"
        if ev.get("tagging_status") not in {"ok", "tagging_unavailable"}:
            return "fail"
    return "pass" if state["evidence_store"] else "fail"


def check_sc2_1(state: AgentState) -> str:
    """SC2-1: each competitor x technology pair must have a TRL row with enough evidence."""
    ...
    for company in competitors:
        for technology in technologies:
            ...
            if len(rows) != 1:
                return "fail"
            row = rows[0]
            if row["evidence_count"] < 2 or row.get("label") not in {"confirmed", "estimated"}:
                return "fail"
    return "pass"
